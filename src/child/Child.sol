...
contract Child is SwapService, DebtService, Ownable {
    ...
    constructor(address _user, address _baseToken, uint _baseTokenDecimals, uint _shaaveLTV) DebtService(_user, _baseToken, _baseTokenDecimals, _shaaveLTV) {}
    ...
    **/
    function short(
        ...
        // Borrow asset
        uint borrowAmount = borrowAsset(_shortToken, _user, _baseTokenAmount);
    ...
    **/
    function reducePosition(
        ...
        (uint amountIn, uint amountOut) = swapToShortToken(_shortToken, baseToken, positionReduction, userPositions[_shortToken].backingBaseAmount, baseTokenConversion);
        ...
        uint debtAfterRepay = getOutstandingDebt(_shortToken) * (10 ** (18 - IERC20Metadata(_shortToken).decimals()));      // Wei, as that's what getPositionGains wants
    ...
    **/
    function withdrawCollateral(uint _amount) public userOnly {
