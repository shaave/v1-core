...
abstract contract DebtService {
    ...
    function borrowAsset(address _shortToken, address _user, uint _baseTokenAmount) internal returns (uint borrowAmount){
        ...
        uint shortTokenConversion = (10 ** (18 - IERC20Metadata(_shortToken).decimals()));
        uint priceOfShortTokenInBase = _shortToken.pricedIn(baseToken);     // Wei
        borrowAmount = ((_baseTokenAmount * baseTokenConversion * shaaveLTV) / 100).dividedBy(priceOfShortTokenInBase, 18) / shortTokenConversion;
    ...
    function repayAsset(address _shortToken, uint _amount) internal {
    ...
    function withdraw(uint _amount) internal {
